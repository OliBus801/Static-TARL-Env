...
def test_flow_consistency(sample_files):
    ...
    assert scenario.check_flow_consistency(np.array([10, 0], dtype=np.int32)) is True
    assert scenario.check_flow_consistency(np.array([5, 0], dtype=np.int32)) is False
    assert scenario.check_flow_consistency(np.array([10, 5], dtype=np.int32)) is False
...
def test_system_cost_requires_consistent_flow(sample_files):
    ...
    scenario.check_flow_consistency(np.array([10, 0], dtype=np.int32))
    total_cost = scenario.calculate_system_cost(np.array([10, 0], dtype=np.int32))
    assert total_cost == 690.0 # f(10) = 60*(1 + 0.15*(10/10)^4) = 69 per unit flow, total 690

    with pytest.raises(ValueError):
        scenario.calculate_system_cost(np.array([0, 0], dtype=np.int32))
